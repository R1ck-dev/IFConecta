...
public class ListarClubesUseCase {
    ...
    public Pagina<ClubeResumoDTO> execute(int pagina, int tamanho) {
        ...
        List<ClubeResumoDTO> resumos = paginaDeClubes.itens().stream()
                .map(clube -> new ClubeResumoDTO(clube.getId(), clube.getNome(), clube.getDescricao(), clube.getMembros().size()))
                .collect(Collectors.toList());

        return new Pagina<>(resumos, paginaDeClubes.paginaAtual(), paginaDeClubes.totalPaginas(), paginaDeClubes.totalItens());
    }
}
